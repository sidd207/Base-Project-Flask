...
class ActivityBase(ABC):
   ...
   def log_partition_payload(self, payload, partition_key=None):
       if partition_key:
           logger.info('Publishing message:: {} with partition key:: {}'.format(
               payload, partition_key))
       else:
           logger.info('Missing partition key on payload: {}'.format(payload))
